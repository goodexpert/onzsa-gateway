...
// Driver Name
const DRIVER_NAME = 'EPSON TM-T82II Receipt5';
...
// Cash Drawer
const CD_KICK_2  = '\x1b\x70\x00\x19\xfa'      // Sends a pulse to pin 2 [] 
const CD_KICK_5  = '\x1b\x70\x01\x19\xfa'      // Sends a pulse to pin 5 [] 
...
const BARCODE_HEIGHT  = '\x1d\x68\x7F' // Barcode Height [1-255]
const BARCODE_WIDTH   = '\x1d\x77\x02' // Barcode Width  [2-6]
//const BARCODE_HEIGHT  = '\x1d\x68' // Barcode Height [1-255]
//const BARCODE_WIDTH   = '\x1d\x77' // Barcode Width  [2-6]
...
module.exports.printBarcode = printBarcode;
module.exports.printImage = printImage;

module.exports.kickOut = kickOut;

var printerName = DRIVER_NAME;
var printText = "";
...
function printBarcode(code, type, width, height, position, font) {
    // Select justification: Centering
    alignCenter();

    try {
        // Set barcode height
        if (height >= 1 && height <= 255) append(BARCODE_HEIGHT);
...
function kickOut(pin) {
    // Select connection pin for opening a cash drawer
    if (pin == 2)
        raw(CD_KICK_2);
    else
        raw(CD_KICK_5);
...
function execute() {
    ...
    printer.printDirect({
        data: printText // or simple String: "some text"
...
function isAvailable() {
    ...
    for (var i = 0; i < printers.length; i++) {
        if (DRIVER_NAME === printers[i].driverName) {
